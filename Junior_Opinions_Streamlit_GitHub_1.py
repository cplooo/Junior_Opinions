...
def Frequency_Distribution(df, column_index, split_symbol=';', dropped_string='沒有工讀', sum_choice=1): ## 當有去掉dropped_string & 是單選題時, sum_choice 要使用 0
    ##### 去掉df在指定的column 'column_index' 中包含 NaN 的 所有rows 並賦值給df_restrict. 
    ####  df本身直接去掉會出現問題, 因為有很多題目都要使用df, df的某個題目的columnu有NA,不代表別題的column也有NA.
    ####  其中 df=df_junior, df_junior_faculty, df_junior_school.
...
# def Draw(系_院_校, column_index, split_symbol=';', dropped_string='沒有工讀', sum_choice=1, result_df=pd.DataFrame(), selected_options=[], dataframes=dataframes, combined_df=combined_df, width1=10,heigh1=6,width2=11,heigh2=8,width3=10,heigh3=6,title_fontsize=15,xlabel_fontsize = 14,ylabel_fontsize = 14,legend_fontsize = 14,xticklabel_fontsize = 14, yticklabel_fontsize = 14, annotation_fontsize = 14, bar_width = 0.1, fontsize_adjust=0, item_name='', rank=False, rank_number=5, df_junior=df_junior, df_junior_faculty=df_junior_faculty, df_junior_school=df_junior_original, desired_order=desired_order, row_rank=False, row_rank_number=3):
def Draw(系_院_校, column_index, split_symbol=';', dropped_string='沒有工讀', sum_choice=1, result_df=pd.DataFrame(), selected_options=[], dataframes=dataframes, combined_df=combined_df, width1=10,heigh1=6,width2=11,heigh2=8,width3=10,heigh3=6,title_fontsize=15,xlabel_fontsize = 14,ylabel_fontsize = 14,legend_fontsize = 14,xticklabel_fontsize = 14, yticklabel_fontsize = 14, annotation_fontsize = 14, bar_width = 0.1, fontsize_adjust=0, item_name='', rank=False, rank_number=5, df_junior=df_junior, df_junior_faculty=df_junior_faculty, df_junior_school=df_junior_original, desired_order=desired_order):    
    ##### 使用Streamlit畫單一圖
    if 系_院_校 == '0':
...
with st.expander("2-2.三年級「上學期」平均每周工讀時數(不列計沒有工讀):"):
    ...
    # Draw(系_院_校, column_index, split_symbol=';', dropped_string='沒有工讀', sum_choice=0, result_df=result_df, selected_options=selected_options, dataframes=dataframes, combined_df=combined_df, width1=10,heigh1=6,width2=11,heigh2=8,width3=10,heigh3=6,title_fontsize=15,xlabel_fontsize = 14,ylabel_fontsize = 14,legend_fontsize = 14,xticklabel_fontsize = 14, yticklabel_fontsize = 14, annotation_fontsize = 14, bar_width = 0.1, fontsize_adjust=0, item_name=item_name, rank=False, rank_number=5, df_junior=df_junior_restrict, df_junior_faculty=df_junior_faculty_restrict, df_junior_school=df_junior_school_restrict, desired_order=desired_order)
    Draw(系_院_校, column_index, split_symbol=';', dropped_string='沒有工讀', sum_choice=0, result_df=result_df, selected_options=selected_options, dataframes=dataframes, combined_df=combined_df, width1=10,heigh1=6,width2=11,heigh2=8,width3=10,heigh3=6,title_fontsize=15,xlabel_fontsize = 14,ylabel_fontsize = 14,legend_fontsize = 14,xticklabel_fontsize = 14, yticklabel_fontsize = 14, annotation_fontsize = 14, bar_width = 0.1, fontsize_adjust=0, item_name=item_name, rank=False, rank_number=5, df_junior=df_junior, df_junior_faculty=df_junior_faculty, df_junior_school=df_junior_original, desired_order=desired_order)    
st.markdown("##")  ## 更大的间隔 
...
with st.expander("4-2.對專業課程提供意見或建議:"):
    ...
    if 系_院_校 == '2':
        ## 使用multiselect组件让用户进行多重选择
        selected_options = st.multiselect('比較選擇: 全校 or 各院：', university_faculties_list, default=['全校','理學院'],key=str(column_index)+'university')
    Draw(系_院_校, column_index, split_symbol=';', dropped_string='沒有工讀', sum_choice=1, result_df=result_df, selected_options=selected_options, dataframes=dataframes, combined_df=combined_df, width1=10,heigh1=6,width2=11,heigh2=8,width3=10,heigh3=6,title_fontsize=15,xlabel_fontsize = 14,ylabel_fontsize = 14,legend_fontsize = 14,xticklabel_fontsize = 14, yticklabel_fontsize = 14, annotation_fontsize = 14, bar_width = 0.1, fontsize_adjust=0, item_name=item_name, rank=True, rank_number=10, df_junior=df_junior, df_junior_faculty=df_junior_faculty, df_junior_school=df_junior_original, desired_order=desired_order)    
st.markdown("##")  ## 更大的间隔
...
###### 7-2.生涯就業輔導: 對畢業後的生涯發展有明確方向與規劃
with st.expander("7-2.對畢業後的生涯發展有明確方向與規劃"):
    # df_junior.iloc[:,54] ## 
    column_index = 54
    item_name = "對畢業後的生涯發展有明確方向與規劃"
    column_title.append(df_junior.columns[column_index][0:])
...
###### 7-3.生涯就業輔導: 瞭解系上學長姐畢業後主要的就業領域與方向
with st.expander("7-3.瞭解系上學長姐畢業後主要的就業領域與方向"):
    # df_junior.iloc[:,55] ## 
    column_index = 55
    item_name = "瞭解系上學長姐畢業後主要的就業領域與方向"
    column_title.append(df_junior.columns[column_index][0:])
...
###### 7-4.生涯就業輔導: 期待畢業學長姊返校與在校同學分享經驗
with st.expander("7-4.期待畢業學長姊返校與在校同學分享經驗"):
    # df_junior.iloc[:,56] ## 
    column_index = 56
    item_name = "期待畢業學長姊返校與在校同學分享經驗"
    column_title.append(df_junior.columns[column_index][0:])
...
###### 7-5.生涯就業輔導: 了解目前國內外產業發展趨勢及就業市場之形勢
with st.expander("7-5.了解目前國內外產業發展趨勢及就業市場之形勢"):
    # df_junior.iloc[:,57] ## 
    column_index = 57
    item_name = "了解目前國內外產業發展趨勢及就業市場之形勢"
    column_title.append(df_junior.columns[column_index][0:])
...
###### 8-1.就學期間:有沒有老師關心您
with st.expander("8-1.就學期間:有沒有老師關心您"):
    # df_junior.iloc[:,59] ## 
    column_index = 59
    item_name = "就學期間:有沒有老師關心您"
    column_title.append(df_junior.columns[column_index][0:])
...
###### 8-2.就學期間:有沒有老師能啟發您, 給您夢想跟動力
with st.expander("8-2.就學期間:有沒有老師能啟發您, 給您夢想跟動力"):
    # df_junior.iloc[:,60] ## 
    column_index = 60
    item_name = "就學期間:有沒有老師能啟發您, 給您夢想跟動力"
    column_title.append(df_junior.columns[column_index][0:])
...
###### 8-3.就學期間:有沒有老師鼓勵您
with st.expander("8-3.就學期間:有沒有老師鼓勵您"):
    # df_junior.iloc[:,61] ## 
    column_index = 61
    item_name = "就學期間:有沒有老師鼓勵您"
    column_title.append(df_junior.columns[column_index][0:])
...
###### 8-4.就學期間:有沒有碰到專心認真上課的老師
with st.expander("8-4.就學期間:有沒有碰到專心認真上課的老師"):
    # df_junior.iloc[:,62] ## 
    column_index = 62
    item_name = "就學期間:有沒有碰到專心認真上課的老師"
    column_title.append(df_junior.columns[column_index][0:])
